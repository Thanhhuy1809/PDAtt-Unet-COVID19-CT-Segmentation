...
- Test data: Dataset3_full.pt (FULL Dataset 1)
"""
...
# FULL Dataset 1
Dataset3_path = os.path.join(base_dir, "Dataset3_full.pt")

# Models trained on Dataset 3
model_src = os.path.join(base_dir, "Model_dataset3_new", "PYAttUNet", "Models")

# Results output
result_dir = os.path.join(base_dir, "Model_D3_new_test_D3", "PYAttUNet", "Results")
...
# Load FULL Dataset 1
##########################################
validate_set = SingleDataset(path=Dataset3_path, transform=val_transforms)
...
for itr in range(runs):
    ...
    with open(result_path, "w") as f:
        print("Run %d | D3 model -> D3 full (cross-dataset)" % itr, file=f)
        print("-" * 10, file=f)
...
if len(F1_mean) > 0:
    ...
    with open(mean_path, "w") as f:
        print("F1_mean   %s" % (F1_mean + [np.mean(F1_mean), stD3]), file=f)
        print("dise_mean %s" % (dise_mean + [np.mean(dise_mean), std2]), file=f)
        print("IoU_mean  %s" % (IoU_mean + [np.mean(IoU_mean), std3]), file=f)

    print("\n========== FINAL RESULTS (D3 model on full D3, cross-dataset) ==========")
    print("F1  : %s   mean=%.4f +-%.4f" % ([round(v, 4) for v in F1_mean], np.mean(F1_mean), stD3))
