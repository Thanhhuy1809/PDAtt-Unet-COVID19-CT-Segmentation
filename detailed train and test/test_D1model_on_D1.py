...
# Models trained on Dataset 1
model_src = os.path.join(base_dir, "Model_dataset1", "PYAttUNet", "Models")

# Results output
result_dir = os.path.join(base_dir, "Model_D1_test_D1", "PYAttUNet", "Results")
...
for itr in range(runs):
    ...
    with open(result_path, "w") as f:
        print("Run %d | D1 model -> D1 full (cross-dataset)" % itr, file=f)
        print("-" * 10, file=f)
...
if len(F1_mean) > 0:
    ...
    print("\n========== FINAL RESULTS (D1 model on full D1) ==========")
