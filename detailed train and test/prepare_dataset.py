...
for itr in range(runs):
    ...
    if itr == 0:  # Chỉ in một lần ở lần chạy đầu tiên
        print("\n" + "="*60)
        print(f"MODEL: {modl_n}")
        print("="*60)

        total_params, trainable_params = count_parameters(model)
        print(f"Tổng số tham số: {total_params:,}")
        print(f"Tham số có thể huấn luyện: {trainable_params:,}")
        print(f"Tham số không huấn luyện: {total_params - trainable_params:,}")
        print(f"Kích thước mô hình (MB): {total_params * 4 / (1024**2):.2f}")

        print("\n" + "-"*60)
        print("Chi tiết từng layer:")
        print("-"*60)
        print(f"{'Layer':<50} {'Params':>15}")
        print("-"*60)
        for name, param in model.named_parameters():
            print(f"{name:<50} {param.numel():>15,}")
        print("-"*60)
